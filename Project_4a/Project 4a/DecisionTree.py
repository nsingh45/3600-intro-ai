...
class Tree:
  ...
  def classify(self, classificationData):
    ...
    """
    n = self.root
    while not hasattr(n, 'value'):
        n = n.children[classificationData[n.attr]]
    return n.value
...
def getClassCounts(examples,className):
    ...
    Returns:
        ...
        to its count.
    """
    classCounts = {}    #dictionary[className] = value
    for item in examples:
        classData = item[className]
        if classData not in classCounts:
            classCounts[classData] = 1
        else:
            classCounts[classData] += 1
    return classCounts
...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    """
    #{possible value of attrName : count of values}
    attributeCounts={}
                                    # find and count values of className
                                    #resulting data structure: {attrValue : {className[a] : count(a), className[b] : count(b), className[c] : count(c)}}
    for value in attrValues:
        attributeCounts[value] = {}
    for item in examples:   #for each item
        if attrName in item and className in item:    #if the attribute is in the item
            attributeKey = item[attrName]   #get the value of the attribute
            classValue = item[className]    #get the value of the target class for this item
            if classValue not in attributeCounts[attributeKey]: #if the value of the target class has not been seen yet
                attributeCounts[attributeKey][classValue] = 1   #main dict[value of known attribute] : {classValue : count]
            else:
                attributeCounts[attributeKey][classValue] += 1
...
def setEntropy(classCounts):
    ...
    """
    numExamples = sum(classCounts)
    total = 0
    for item in classCounts:
        prob = float(item)/float(numExamples)
...
def remainder(examples,attrName,attrValues,className):
    ...
    attCounts = getAttributeCounts(examples, attrName, attrValues, className)
    total = 0
    numExamples = len(examples)
    for item in attCounts:
        examplesWithAtt = attCounts[item]
        exampleVals = examplesWithAtt.values()
        itemEntropy = setEntropy(exampleVals)
        examplesInSubset = len(exampleVals)
        if examplesInSubset > 1:
            examplesInSubset = sum(exampleVals)
        total += (float(examplesInSubset)/float(numExamples)) * itemEntropy
    return total
...
def giniIndex(classCounts):
    ...
    """
    numExamples = len(classCounts)
    if numExamples > 1:
        numExamples = sum(classCounts)
    elif numExamples == 1:
        numExamples = classCounts[0]
    total = 1
    for item in classCounts:
        probItem = float(item) / float(numExamples)
        probItem *= probItem
        total -= probItem
    return total
...
def giniGain(examples,attrName,attrValues,className):
    ...
    """
    classCounts = getClassCounts(examples, className)
    classCountList = classCounts.values()
    numExamples = sum(classCountList)
    attCounts = getAttributeCounts(examples, attrName, attrValues, className)
    giniIndices = giniIndex(classCountList)
    itemGini = []
    for item in attCounts:
        vals = attCounts[item].values()
        weight = 0
        if len(vals) == 1:
            weight = vals[0]
        elif len(vals) > 1:
            weight = sum(vals)
        weight = float(weight) / float(numExamples)
        itemGini.append(weight *  giniIndex(vals))
    itemGini = sum(itemGini)
    if itemGini == 0:
        return sys.maxint
    return 1/itemGini
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    """
    pluralityValue = getMostCommonClass(examples, className)
    classCounts = getClassCounts(examples, className)
    classCountList = classCounts.values()
    if not len(examples):
        return LeafNode(defaultLabel)
    elif len(examples) in classCountList:
        return LeafNode(pluralityValue) #we'll see if i need to modify this
    elif not len(remainingAttributes):
        return LeafNode(pluralityValue)
    else:
        gainScores = {}
        maxGain = -1.0
        maxGainAtt = ''
        for r in remainingAttributes:
            gainScores[r] = gainFunc(examples, r, attributeValues[r], className)
            gainScore = gainFunc(examples, r, attributeValues[r], className)
            if gainScore > maxGain:
                maxGain = gainScore
                maxGainAtt = r
        newNode = Node(maxGainAtt)
        subtreeDefault = pluralityValue
        attributeCopy = attributeValues
        for v in attributeValues[maxGainAtt]:
            subtreeExamples = getPertinentExamples(examples, maxGainAtt, v)
            subtreeAttributes = copy.deepcopy(remainingAttributes)
            subtreeAttributes.remove(maxGainAtt)
            subtree = makeSubtrees(subtreeAttributes, subtreeExamples, attributeCopy, className, pluralityValue, setScoreFunc, gainFunc)
            newNode.children[v] = subtree
        return newNode
