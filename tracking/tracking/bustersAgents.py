...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        ghostPositionsandDistances = util.Counter()
        for pos in livingGhostPositionDistributions:
            ghostPositionsandDistances[pos.argMax()] = 0.0 #max confidence location of each ghost
        for pos in ghostPositionsandDistances:
            ghostPositionsandDistances[pos] = self.distancer.getDistance(pacmanPosition, pos)
        closestGhost = ghostPositionsandDistances.sortedKeys()[-1]
        ghostDistance = ghostPositionsandDistances[closestGhost]
        actionsWithDistances = util.Counter()
        for a in legal:
            result = Actions.getSuccessor(pacmanPosition, a)
            actionsWithDistances[a] = self.distancer.getDistance(result, closestGhost)
        return actionsWithDistances.sortedKeys()[-1]
