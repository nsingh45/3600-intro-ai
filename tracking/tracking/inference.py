...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        ghostJail = self.getJailPosition()
        ...
        eatenFlag = False
        if noisyDistance == None:   #ghost is eaten; put it in jail
            allPossible[ghostJail] = 1
            eatenFlag = True
        #iterate through all legal positions
        counter = 0
        for p in self.legalPositions:
            if eatenFlag:   #if the ghost is eaten, the possibility of it being in a legal position is 0
                allPossible[p] = 0
            else:
                trueDistance = util.manhattanDistance(p, pacmanPosition)    #actual distance from pacman's state to state p
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        timeUpdate = util.Counter()
        pacmanPosition = gameState.getPacmanPosition()
        #what is the previous distribution?
        for p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos in newPosDist:
                timeUpdate[newPos] += self.beliefs[p] * newPosDist[newPos]
        """
        for p in self.legalPositions:
            if eatenFlag:   #if the ghost is eaten, the possibility of it being in a legal position is 0
                allPossible[p] = 0
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        particlesPerLocation = self.numParticles/numLocations
        
        for location in locations:  #for each location
            #put particlesPerLocation particles here
            for i in range(0, particlesPerLocation):
                self.particleList.append(location)
        """
        print self.numParticles
        print numLocations, 'num locations'
        print particlesPerLocation, 'particles per location'
    ...
    def observe(self, observation, gameState):
        ...
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance)   #P(X | e)
        pacmanPosition = gameState.getPacmanPosition()
        ghostJail = self.getJailPosition()
        beliefDist = self.getBeliefDistribution()   #Counter with (position) : P(position)
        weightedDist = util.Counter()
        eatenFlag = False
        if noisyDistance == None:   #ghost is eaten; put it in jail
            weightedDist[ghostJail] = 1
            eatenFlag = True
        #iterate through all particles in list
        else:
            for i in range(0, len(self.particleList)):
                trueDistance = util.manhattanDistance(self.particleList[i], pacmanPosition)    #actual distance from pacman's state to state p
                if emissionModel[trueDistance] > 0:
                    weightedDist[self.particleList[i]] += beliefDist[self.particleList[i]] * emissionModel[trueDistance]

        for key in weightedDist:
            if beliefDist[key] != 0:
                weightedDist[key] *= (self.numParticles/beliefDist[key])
            weightedDist.normalize()
        #print weightedDist, 'weighted dist at iteration', self.counter
        if len(weightedDist) == 0:
            self.initializeUniformly(gameState)
        else:
        #now resample
            newSample = []
            for i in range(0, self.numParticles):
                newSample.append(util.sample(weightedDist))
            self.particleList = newSample
        self.counter += 1
    ...
    def elapseTime(self, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        for i in range(0, len(self.particleList)):
            particlePos = self.particleList[i]
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particlePos))
            newSamples.append(util.sample(newPosDist))
        self.particleList = newSamples
